...
_COMPCARS_DATASET_DIR = './datasets/compcars/'
_COMPCARS_LOWSHOT_BENCHMARK_CATEGORY_SPLITS_PATH = './data/COMPCARS_LOWSHOT_BENCHMARK_CATEGORY_SPLITS.json'
...
class CompCars(data.Dataset):
    def __init__(
        ...
        self.name = 'CompCars_Split_' + split

        data_dir = _COMPCARS_DATASET_DIR
        print('==> Loading CompCars dataset - split {0}'.format(split))
        print('==> CompCars directory: {0}'.format(data_dir))
...
class CompCarsLowShot(CompCars):
    def __init__(
        self,
        phase='train',
        split='train',
        ...
        self.phase = phase
        self.split = split
        self.name = 'CompCarsLowShot_Phase_' + phase + '_Split_' + split
        print('==> Loading CompCars dataset (for few-shot benchmark) - phase {0}'.
            format(phase))

        #***********************************************************************
        with open(_COMPCARS_LOWSHOT_BENCHMARK_CATEGORY_SPLITS_PATH, 'r') as f:
...
class CompCarsLowShotFeatures:
    def __init__(
        ...
        self.name = (f'CompCarsLowShotFeatures_ImageSplit_{self.image_split}'
                     f'_Phase_{self.phase}')

        dataset_file = os.path.join(
            data_dir, 'CompCars_' + self.image_split + '.h5')
        ...
        with open(_COMPCARS_LOWSHOT_BENCHMARK_CATEGORY_SPLITS_PATH, 'r') as f:
            label_idx = json.load(f)
        base_classes = label_idx['base_classes'] # combination of base_classes_1 and base_classes_2
...
class CompCarsFeatures:
    def __init__(
        ...
        self.split = split
        self.name = (f'CompCarsFeatures_ImageSplit_{self.split}')

        dataset_file = os.path.join(
            data_dir, 'CompCars_' + self.split + '.h5')
        self.data_file = h5py.File(dataset_file, 'r')
        self.count = self.data_file['count'][0]
        self.features = self.data_file['all_features'][...]
        self.labels = self.data_file['all_labels'][:self.count].tolist()
